...
import { DEFAULT_MAX_STEPS, MAX_STEPS_ACTOR, MAX_STEPS_THINKER, MODEL_ACTOR, MODEL_THINKER } from './consts.js';
import { ValueError } from './errors.js';
import getLogger from './logger.js';
import { buildPrompt } from './utils/index.js';
import type { ChatCompletionContentPart, ChatCompletionMessageParam } from 'openai/resources.js';
...
export default class Actor {
  ...
  async initTask(
    ...
    if (maxSteps > limit) {
      logger.warn(`max_steps (${maxSteps}) exceeds limit for model '${this.model}'. Capping to ${limit}.`);
      maxSteps = limit;
    }
